...
| **Date** | 2026-06-20 |
| **Status** | Draft for Vendor Review (gap-closure revision) |
...
| 1.1 | 2026-06-20 | Rqeem BA | Gap-closure revision. Added template versioning and archival, MFA for privileged roles, invite/admin-approved account activation, immediate session revocation, upload signature and malware validation, export formula-injection neutralization, optimistic concurrency, and configurable status thresholds. Corrected the delayed-status no-document logic. Audited privileged personal-data reads. Closed OQ-7. |
...
- Authenticated, role-based access for four user classes, with multi-factor authentication required for privileged roles.
- Invite/administrator-approved account activation: self-registered accounts remain pending until a manager activates and links them to a project.
...
- Versioned, configurable stage templates ("pipelines") composed of ordered stages and typed data-capture fields, including image/photo capture. In-flight projects stay pinned to the template version they were assigned.
...
| **MFA** | Multi-factor authentication: a second proof of identity beyond the password. |
| **TOTP** | Time-based one-time password (authenticator-app second factor). |
| **Template version** | An immutable snapshot of a stage template's stages and fields; in-flight projects stay pinned to the version assigned to them. |
| **Pending account** | A self-registered account that cannot access project features until a manager activates and links it. |
...
- **DC-8** Stage templates shall be versioned. A template version assigned to a project is immutable for that project; editing a template that has been assigned produces a new version and never mutates the version a live project is pinned to.
- **DC-9** Multi-factor authentication shall be enforced for privileged roles (Project Manager, Administrator) at the application tier.
- **DC-10** Authorization state shall be revocable in real time: a deactivated or deleted account shall be denied on its next request regardless of any unexpired access credential.
...
- **FR-AUTH-001** The System shall allow an unauthenticated visitor to self-register a new account by providing full name, email address, password, and company name. A self-registered account shall be created in a **pending** state and shall not gain access to any project feature until a manager activates and links it (FR-USER-014).
- **FR-AUTH-002** The System shall require the registration password to be at least 12 characters and at most 128 characters, shall permit passphrases (spaces allowed), and shall reject passwords found in a known-breach list.
...
- **FR-AUTH-005** The System shall assign every self-registered account the **Investor** role in a **pending** state, and shall never accept or honor a role value supplied in a registration request.
...
- **FR-AUTH-010a** The System shall deny project access to a **pending** (not-yet-activated) account and shall return a distinct "account pending activation" message, while permitting sign-in only to the limited pending state.
...
- **FR-AUTH-022** The System shall require multi-factor authentication for Project Manager and Administrator accounts and shall offer it as an optional control to Investor and Top Management accounts. A privileged account that has not completed MFA enrollment shall be limited to the enrollment flow until enrollment is complete.
- **FR-AUTH-023** The System shall implement the second factor as a time-based one-time password (TOTP) compatible with standard authenticator applications, and shall verify it after successful password verification before establishing a full session.
- **FR-AUTH-024** The System shall issue a set of single-use recovery codes at MFA enrollment, shall allow a recovery code to substitute for the TOTP once, and shall allow regeneration of recovery codes by the account holder.
- **FR-AUTH-025** The System shall store MFA secrets and recovery codes only in a protected (encrypted) form and shall never return them after enrollment display.
- **FR-AUTH-026** The System shall allow an Administrator to reset (re-enroll) a user's MFA, recorded in the audit log.
- **FR-AUTH-027** The System shall re-validate the acting account's active, non-deleted state on every authenticated request and shall deny the request immediately if the account is deactivated or deleted, regardless of an unexpired access credential.

**Workflow — Sign-in**

- Preconditions: visitor is unauthenticated; account exists and is active.
- Main flow: visitor submits email + password → System validates throttle → verifies credentials → if MFA is enrolled or required, prompts for and verifies the TOTP (or a recovery code) → establishes session → returns access credential, sets renewal cookie, returns user profile.
- Alternate/exception: invalid credentials → 401 generic error; deactivated account → 403; pending (un-activated) account → 403 "pending activation"; privileged account without MFA enrolled → routed to MFA enrollment; failed/absent second factor → 401; throttle exceeded → 429.
...
- **FR-PROJ-012** The System shall, when a project's pipeline is changed, reset the project's current stage to the first stage of the newly assigned pipeline version and reset the project's construction percentage to that first stage's baseline percentage.
...
- **FR-PROJ-016** The System shall pin each project to the specific **stage-template version** assigned to it. Subsequent edits to that template (which create new versions, FR-TPL-016) shall not alter a live project's stages, fields, or current stage unless the project is explicitly reassigned.
- **FR-PROJ-017** The System shall render a project's historical status-update records using the stage labels of the template version in force when each record was created, including labels belonging to archived versions.
- **FR-PROJ-018** The System shall apply optimistic concurrency control to project metadata edits using a version token, and shall reject a write based on a stale token with a conflict result so a later edit cannot silently overwrite an intervening one.
...
- **FR-TPL-011** The System shall apply template edits with replace-on-save semantics: the submitted set of stages and fields fully replaces the prior set, with order indices assigned by submitted position. When the template has ever been assigned to a project, replace-on-save shall produce a **new template version** (FR-TPL-016) rather than mutating an assigned version.
...
- **FR-TPL-015** The System shall allow a manager to delete a template **only when no version of it has ever been assigned to a project**. A template that has been assigned shall not be hard-deleted; it shall instead be **archived**.
- **FR-TPL-016** The System shall version stage templates. Each saved edit of an assigned template creates a new, immutable version; prior versions are retained for the projects pinned to them. New project assignments use the latest non-archived version.
- **FR-TPL-017** The System shall allow a manager to **archive** a template, excluding it from assignment selection for new projects while keeping every version resolvable for the historical display of projects and update records that reference it.
- **FR-TPL-018** The System shall reject assignment of an archived template version to a project.
...
- **FR-UPD-016** The System shall reject a second review action on an update that has already been approved or rejected, and shall enforce this atomically so that two concurrent reviews of the same pending update cannot both take effect.
...
- **FR-DOC-010** The System shall count a document toward a project's "current-stage document present" check only when it belongs to an **approved** update whose target stage equals the project's current stage. This check shall be evaluated only for current stages that define at least one required document or image field; a stage with no such field is treated as not requiring a current-stage document.
- **FR-DOC-011** The System shall validate each uploaded file's binary signature (magic bytes) against its declared content type and shall reject a file whose actual format does not match an allow-listed type, regardless of the declared content type or file extension.
- **FR-DOC-012** The System shall scan each uploaded file for malware before it is persisted or made available for download, and shall reject or quarantine any file that fails the scan.
...
- **FR-NOTIF-014** The System shall offer Top Management an opt-in notification for projects that reach **stalled** derived status, off by default and controlled per user, delivered through the same in-application channel.
...
- **FR-DASH-008** The System shall neutralize spreadsheet formula injection in tabular exports by prefixing any exported cell value beginning with `=`, `+`, `-`, `@`, tab, or carriage return so that it is rendered as text and not executed by a spreadsheet application.
...
- **FR-STAT-004** The System shall classify a project as **stalled** when its current stage's category is "on-hold," when it has no assigned stage, or when the time since its last approved update exceeds the configured **stalled threshold** (from the notifications setting, FR-SET-003; default 45 days when unset).
- **FR-STAT-005** The System shall classify a project as **delayed** when the time since its last approved update exceeds the configured **delayed threshold** (from the notifications setting, FR-SET-003; default 30 days when unset), or when the current stage defines a required document/image field and there is no approved current-stage document on file (FR-DOC-010).
...
| 2 | current stage category = on-hold, OR no stage assigned, OR days-since-last-approved-update > stalled threshold (default 45) | stalled |
| 3 | days-since-last-approved-update > delayed threshold (default 30), OR (current stage requires a document AND no approved current-stage document) | delayed |
...
- **FR-USER-014** The System shall allow a Project Manager to activate a **pending** Investor account and link it to a project, and shall allow an Administrator to activate any pending account. Until activated, a pending account shall not access project features. Activation shall be recorded in the audit log.

**User account lifecycle**

```mermaid
stateDiagram-v2
  [*] --> Pending: self-registered (investor)
  [*] --> Active: created by PM (investor only) / Admin (any role)
  Pending --> Active: manager activates and links (PM: investor / Admin: any)
  ...
  Pending --> Deleted: Administrator deletes
  Deleted --> [*]
```
...
- **FR-AUDIT-002** The System shall record audit entries for at least: user created, user updated, user activated, user credentials provisioned, MFA enrolled, MFA reset, user deleted, project created, project assigned (investor), project pipeline assigned, project attention-flag changed, project deleted, status update approved, status update rejected, stage template version created, stage template archived, stage template deleted, settings updated, and document deleted.
- **FR-AUDIT-002a** The System shall record an audit entry when investor personal data is read by a privileged role in a disclosing context — specifically the viewing of investor contact details (FR-PROJ-007) and the generation of a portfolio export (FR-DASH-006) — capturing actor, the data category accessed, and timestamp, to evidence personal-data access under PDPL.
...
- **FR-SET-003** The System shall provide a notifications setting containing an out-of-band-notification enabled flag, a stalled-alert threshold in days, and a delayed-alert threshold in days. Both thresholds shall drive the derived-status engine (FR-STAT-004, FR-STAT-005).
...
> Note (resolved in v1.1): the stored stalled-alert threshold, and a new delayed-alert threshold, now drive the derived-status engine (FR-STAT-004/005). The fixed-constant behavior of the reference build is superseded. OQ-7 is closed.

### 4.16 System Health (SYS)

- **FR-SYS-001** The System shall expose an unauthenticated health-status endpoint reporting only service liveness, disclosing no version string and no sensitive data. Version and build identity shall be available only on an authenticated or internal-network monitoring path.
...
| --- | --- | --- |
| **Sign-in / Register** | Authenticate or self-register | Email, password (sign-in); name, email, company, password (register, password ≥ 12, creates a pending account). MFA prompt (TOTP or recovery code) for enrolled/required users; MFA enrollment screen for privileged users; "pending activation" notice for un-activated accounts. Forgot-password entry point (target — OQ-2). |
...
| **Stage Templates** | Manage pipelines | List templates with version and default flag; builder for stages (name, category, progress %, description) and fields (label, base type, widget, required, options) with reorder and live field preview. Saving an in-use template creates a new version; archive (not delete) for templates ever assigned. |
| **Users** | Manage accounts | List/search/filter; pending-account queue with activate-and-link (PM: investor only; Admin: any role); create account (PM: investor only; Admin: any role) with one-time password display; edit / reset password / reset MFA / delete (Administrator only). |
...
The reference build contains no external system integrations. Interfaces are limited to the internal/business interfaces below; IF-005 and IF-007 are target integrations introduced by this specification.
...
| **IF-007 (target) Malware scanning** | Screen uploaded files before persistence | File content out; verdict in | Application ⇄ Scanning service | On every file upload (FR-DOC-012; in-Kingdom service — OQ-14) |
...
- **NFR-AVL-005** The System shall apply optimistic concurrency control to project metadata edits and to update review, so that a stale write or a concurrent second review is rejected with a conflict rather than silently lost (FR-PROJ-018, FR-UPD-016).
...
- **NFR-SEC-019** The System shall enforce multi-factor authentication (TOTP) for Project Manager and Administrator accounts (FR-AUTH-022..026), shall store MFA secrets and recovery codes encrypted, and shall offer optional MFA to all other roles.
- **NFR-SEC-020** The System shall enforce the password policy of FR-AUTH-002 (minimum 12 characters, passphrases permitted, known-breach rejection) for all password creation and reset operations.
...
- **NFR-SEC-021** The System shall revoke access in real time: each authenticated request shall re-confirm the account is active and not deleted, and shall be denied immediately otherwise, never relying solely on access-credential expiry (FR-AUTH-027).
- **NFR-SEC-022** The System shall validate uploaded file signatures against declared content types and shall scan uploads for malware before persistence (FR-DOC-011, FR-DOC-012).
- **NFR-SEC-023** The System shall neutralize spreadsheet formula injection in all tabular exports (FR-DASH-008).
...
- **CMP-004** The solution shall align with the NCA Essential Cybersecurity Controls (ECC), including governance, identity and access management (including multi-factor authentication for privileged access, NFR-SEC-019), data protection, cryptography, logging and monitoring, vulnerability management, and incident handling.
...
- **CMP-012** The solution shall log privileged reads of investor personal data (contact-detail views and portfolio exports) in the audit log (FR-AUDIT-002a) to evidence access for PDPL and NCA review.
...
| --- | --- | :---: | :---: | :---: | :---: |
| 1 | Self-register (creates pending investor account) | ✓ | ✓ | ✓ | ✓ |
...
| 27 | View own notifications | ✓ | (opt-in stalled only) | ✓ | ✓ |
| 28 | Multi-factor authentication | optional | optional | required | required |
| 29 | Activate pending account / link investor | ✗ | ✗ | ✓ (investor only) | ✓ (any role) |
| 30 | View audit log including personal-data reads | ✗ | ✗ | ✗ | ✓ |

Notes:
- Row 27: in v1.1, notification events target investors and managers; Top Management receives no notifications except the opt-in stalled-project alert (FR-NOTIF-014).
- Row 7: an investor may hold at most one pending update per project at a time.
- Row 28: privileged accounts cannot reach project features until MFA enrollment is complete.

---

## 10. Business Rules

- **BR-001** Self-registration always creates a **pending** Investor; the role is never taken from the request, and the account gains access only after a manager activates and links it.
- **BR-002** Email is unique and stored lower-case; it is the login identifier and cannot be changed via self-service.
- **BR-003** Registration password length is 12–128 characters, passphrases permitted, and known-breach passwords are rejected.
...
- **BR-010** Assigning or changing a project's pipeline sets the current stage to that pipeline's first stage and resets construction percentage to that stage's baseline; the project stays pinned to the assigned template version.
...
- **BR-023** Complete = current stage category "complete"; Stalled = category "on-hold" or no stage or last approved update older than the configured stalled threshold (default 45 days); Delayed = last approved update older than the configured delayed threshold (default 30 days) or (the current stage requires a document and no approved current-stage document exists); otherwise On-track.
...
- **BR-028** Stage-template edits replace the prior stage/field set entirely (replace-on-save); order follows submitted position. Editing a template that has ever been assigned creates a new immutable version, and projects stay pinned to the version they were assigned.
...
- **BR-043** Multi-factor authentication is required for Project Manager and Administrator accounts and optional for Investor and Top Management; privileged accounts cannot reach project features until MFA enrollment is complete.
- **BR-044** A deactivated or deleted account is denied on its next request, regardless of an unexpired access credential.
- **BR-045** A template that has ever been assigned to a project cannot be hard-deleted; it is archived. Archived templates cannot be assigned to new projects but remain resolvable for historical display.
- **BR-046** Construction percentage and current stage on pipeline reassignment reset to the new first stage's baseline; historical update records keep their original stage labels.
- **BR-047** Project metadata edits and update reviews use optimistic concurrency; stale writes and concurrent second reviews are rejected with a conflict.
- **BR-048** Uploaded files are validated by binary signature against the allow-list and scanned for malware before persistence; a signature mismatch or scan failure is rejected.
- **BR-049** Tabular exports neutralize spreadsheet formula injection on any cell beginning with `=`, `+`, `-`, `@`, tab, or carriage return.
- **BR-050** Privileged reads of investor personal data (contact-detail views and exports) are recorded in the audit log.
- **BR-051** The configured stalled and delayed thresholds drive the derived-status engine.
- **BR-052** Top Management receives no notifications except an opt-in stalled-project alert.
...
- A new visitor can self-register and is created as a pending Investor regardless of any submitted role, with no project access until activated.
- Sign-in with valid active credentials succeeds; invalid credentials return one generic error; a deactivated account is refused; a pending account is refused with a distinct message; exceeding the attempt threshold returns a throttle error.
- A privileged account must complete MFA enrollment before reaching project features; at sign-in it must pass TOTP (or a one-time recovery code); MFA secrets are never returned after enrollment.
- A deactivated or deleted account is denied on its next request even within the access-credential lifetime.
...
**AC-ACTIVATE — Account activation**
- A pending self-registered account cannot access projects; a Project Manager can activate and link a pending Investor; an Administrator can activate any pending account; activation is audited.
...
- Editing a template fully replaces its stages/fields; editing a template that has been assigned creates a new version and leaves live projects pinned to their original version; only one template can be default.
- A template that has ever been assigned cannot be hard-deleted and can only be archived; archived templates cannot be assigned to new projects but still resolve for historical display.
...
- Approval advances the project; rejection records a reason and leaves the project unchanged; a stale/backward/foreign-pipeline approval is refused; an already-reviewed update cannot be reviewed again, and two concurrent reviews cannot both take effect.
...
**AC-DOC — Documents**
- Uploads above the size limit or of disallowed types are rejected; a file whose binary signature does not match an allow-listed type is rejected; a file failing the malware scan is rejected or quarantined; only users with access to the parent project can download; Administrators can list and delete any document.
...
- The dashboard shows correct key figures and supports search and filtering; the displayed status matches the server-computed value for all four status outcomes; the configured stalled and delayed thresholds drive the engine; a stage with no required document does not force a delayed status; managers can export the portfolio, and exported cells are neutralized against formula injection.
...
- Every defined privileged action produces an immutable audit entry visible only to Administrators; privileged reads of investor personal data (contact-detail views, exports) are recorded; settings are readable/writable only by Administrators.
...
| --- | --- | --- |
| FR-AUTH-001..027 | Authentication, session & MFA | AC-AUTH |
| FR-USER-014 | Account activation | AC-ACTIVATE |
| FR-RBAC-001..008; NFR-SEC-007..010 | Authorization | AC-RBAC |
| FR-PROJ-001..018 | Projects | AC-PROJ |
| FR-TPL-001..018 | Stage templates | AC-TPL |
| FR-UPD-001..020 | Updates & review | AC-UPD |
| FR-FLD-001..010 | Field capture | AC-FLD |
| FR-DOC-001..012 | Documents | AC-DOC |
| FR-MSG-001..006 | Messaging | AC-MSG / AC-NOTE |
| FR-NOTE-001..005 | Internal notes | AC-MSG / AC-NOTE |
| FR-NOTIF-001..014 | Notifications | AC-NOTIF |
| FR-DASH-001..008; FR-STAT-001..008 | Dashboard & status | AC-DASH / AC-STAT |
| FR-USER-001..014 | User administration | AC-USER |
| FR-AUDIT-001..005; FR-SET-001..004 | Audit & settings | AC-AUDIT / AC-SET |
| FR-SYS-001 | Health | AC-DASH / AC-STAT (monitoring) |
| OE-3..7; CMP-001..012; DR-004; NFR-SEC-011/013/019..023 | Compliance & residency | AC-COMP |
...
| OQ-7 | Configurable stalled threshold | Resolved in v1.1: the configured stalled and delayed thresholds now drive the status engine (FR-STAT-004/005, FR-SET-003). | Resolved |
...
| OQ-13 | MFA method and recovery | v1.1 specifies TOTP plus single-use recovery codes for privileged roles. Confirm whether an additional channel (e.g., SMS/email OTP) is required and the Admin-assisted recovery process. | Needs confirmation |
| OQ-14 | Malware-scanning service | v1.1 requires upload malware scanning (FR-DOC-012). Confirm an in-Kingdom scanning engine/service that satisfies data-residency (OE-6). | Needs confirmation |
| OQ-15 | Template-version migration | v1.1 pins live projects to their assigned template version and does not auto-migrate them on template edits. Confirm whether a controlled, manager-initiated migration path to a newer version is required. | Needs confirmation |
| OQ-16 | Pending-account activation linkage | v1.1 requires a manager to activate and link self-registered pending accounts. Confirm the matching basis (e.g., email/agreement number) and whether unlinked pending accounts expire. | Needs confirmation |
